...
window.onload = function () {
    ...
    var tags = ["Chanel", "Lancome", "Chloe", "D&G", "YSL"];
...
function phanTich_URL_chiTietSanPham() {
    ...
    var s = addThongSo('Nhãn hiệu', sanPhamHienTai.detail.screen);
    s += addThongSo('Giới tính', sanPhamHienTai.detail.os);
    s += addThongSo('Xuất xứ', sanPhamHienTai.detail.camara);
    s += addThongSo('Phong cách', sanPhamHienTai.detail.camaraFront);
    s += addThongSo('Năm phát hành', sanPhamHienTai.detail.cpu);
    s += addThongSo('Nồng độ', sanPhamHienTai.detail.ram);
    s += addThongSo('Dung tích', sanPhamHienTai.detail.rom);
    s += addThongSo('Nhóm hương', sanPhamHienTai.detail.microUSB);
    s += addThongSo('Độ lưu hương', sanPhamHienTai.detail.battery);
    info.innerHTML = s;
    ...
    // Hình nhỏ
    addSmallImg("img/products/Chloe-Love-Story-For-Women-600x600.jpg");
    addSmallImg("img/chitietsanpham/D&G-Light-Blue-Eau-Intense-1.jpg");
    addSmallImg("img/chitietsanpham/D&G-Light-Blue-Eau-Intense-2.jpg");
    addSmallImg("img/chitietsanpham/D&G-Light-Blue-Eau-Intense-3.jpg");
    addSmallImg("img/products/Chloe-Love-Story-For-Women-600x600.jpg");
    addSmallImg("img/chitietsanpham/D&G-Light-Blue-Eau-Intense-3.jpg");
    addSmallImg("img/products/Chloe-Love-Story-For-Women-600x600.jpg");
...
function getDetailPromo(sp) {
    switch (sp.promo.name) {
        ...
        case 'giamgia':
            ...
            return `Khách hàng sẽ được giảm ` + span + `₫ khi mua trực tiếp tại cửa hàng`;

        case 'moiramat':
            return `Khách hàng được tặng Voucher khi mua trực tiếp tại cửa hàng.`;

        case 'giareonline':
            var del = stringToNum(sp.price) - stringToNum(sp.promo.value);
            var span = `<span style="font-weight: bold">` + numToString(del) + `</span>`;
            return `Sản phẩm sẽ được giảm ` + span + `₫ khi mua hàng online bằng thẻ VPBank hoặc Momo`;

        default:
            var span = `<span style="font-weight: bold">Voucher giảm 50%</span>`;
            return `Cơ hội nhận được ` + span + ` cho lần mua tiếp theo khi trả góp Home Credit`;
    }
}
